...
# ------------ auxiliary function converting alpha and beta_0 to beta diff
#
...
# ------------ update of beta_0 (single kernel)
...
# beta_0 is a p-element vector
# m_0, s0_sq, s_sq: hyperparameters
...
single_beta_0_update <- function(beta_diff,
  ...
  common_var <- 1/(1/s0_sq + nrow(beta_diff)/s_sq)
...
# beta_0_l is a value of beta_0 for field l
...
# ------------  function for performing the full update on beta_0, alpha and (equivalently) beta_diff
...
# theta is a list of vectors theta_l for l in 1:p used in compute_loglikelihood_one_cluster_one_field_cpp
# partition_ll is an n x p matrix with current log-likelihood values for a given cluster and field
# m0, s0_sq and s_sq are hyperparameters
# proposal_sd is the std of the Gaussian proposal in the Metropolis-within-Gibbs update
# p is the number of columns of V (variables)
# n is the number of rows of V (observations)
# returns a list with the the updated matrix beta_diff and the corresponding matrix alpha, and updated beta_0
...
single_full_alpha_update <- function(beta_diff,
                                     ...
                                     n){
  # updating beta_0 
  beta_0_update <- single_beta_0_update(beta_diff, beta_0, m0, s0_sq, s_sq)
  ...
  return(list(beta_diff = new_beta_diff, alpha = new_alpha, beta_0 = beta_0, acc_matrix = acc_matrix))
...
single_full_alpha_update_cl_size <- function(beta_diff,
  ...
  beta_0 <- unlist(result)
  
  # matrices for storing new values
  new_beta_diff <- matrix(NA, ncol = p, nrow = n)
  new_alpha <- matrix(NA, ncol = p, nrow = n)
  acc_matrix <- matrix(NA, ncol = p, nrow = n)
  ## update each beta_diff
  for (l in 1:p){
    theta_l <- theta[[l]]
    beta_0_l <- beta_0[l]
    for (icluster in 1:n){
      # update of beta prime and simultaneoulsy alpha
      beta_diff_j_l = beta_diff[icluster, l]
      ...
      if (is.na(partition_ll[icluster, l])){
        ...
        #                                                   s_sq)
        # # we use beta_diff hence the mean is 0, not beta_0
        logPrior <- dnorm(proposed_state, 0, sqrt(s_sq), log = TRUE)
        logLik <- compute_loglikelihood_one_cluster_one_field_cpp(l-1, icluster-1, clustering,
                                                                  theta_l, V-1, proposed_alpha_j_l)
        proposed_density <- logLik + logPrior
        accept <- (logu < (proposed_density - current_density))
        ...
        result_list <- list(beta_diff_j_l = current_state, alpha_j_l = alpha_j_l, accept = accept)
      }
