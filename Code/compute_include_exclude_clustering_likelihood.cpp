...
// [[Rcpp::export]]
List compute_include_exclude_cpp(const int  j,
	...
	NumericMatrix logprob_include(n,p);
	NumericMatrix logprob_exclude(n,p);		
	...
	for (int q = 0; q < n; q++){
		...
		if (clsize[q] == 0 || (clsize[q] == 1 && eta[j] == q) ){
			for (int l = 0; l < p ; l++ ){
				...
				logprob_include(q,l) = log(theta_field[V(j,l)]);
				logprob_exclude(q,l) = NA_REAL;
				lpsampq[q] += log(theta_field[V(j,l)]);
			}
			logprob_newcluster[q] = log(double(N - ksize));
			...
			for (int l = 0; l < p; l++){
				...
				for (int imember = 0; imember < clsize[q]; imember++){
					...
					if (i != j){// if i is another member in the same cluster with j 
						logprod += log( (1 - alpha(q,l)) * (V(i,l) == V(j,l)) + alpha(q,l) * theta_field[V(i,l)]);
		...
		if (clsize[q] > 0 && eta[j] != q){
			for (int l = 0; l < p ; l++){
				...
				logprod += log(theta_field[V(j,l)]);
				logprod += log( 1 - alpha(q,l));
				double max_logs = std::max(logprod, lprob_withoutj + log(alpha(q,l)) + log(theta_field[V(j,l)]));
				lprob_withj = max_logs + log(exp(logprod - max_logs) + exp(lprob_withoutj + log(alpha(q,l)) + log(theta_field[V(j,l)]) - max_logs));
				logprob_include(q,l) = lprob_withj;
				logprob_exclude(q,l) = lprob_withoutj;
				lpsampq[q] += lprob_withj - lprob_withoutj;
			}
	...
	return List::create(Named("logprob_include") = logprob_include, 
						Named("logprob_exclude") = logprob_exclude, 
						Named("logprob_newcluster") = logprob_newcluster, 
						Named("psampq") = psampq);
}