...
void  rlambda11_alt(int Nlambda[],
  int lambda[],
  double p[],
  int V[],
  int dime[],
  ...
  int clsize[n];
  double psamp[n], pcluster_M[n*H],  prob_M[n*H], pcluster_M_new[n*H],  prob_M_new[n*H], pcluster_M_old[n*H],  prob_M_old[n*H];
  double logpcond_a_old[n*H], logpcond_a_new[n*H], logpcond_aSC_new[n*H],logpcond_aSC_old[n*H], a_prop[H*n],logita[H*n],logitadif[H*n],logitadif_prop[H*n];;
  ...
  for (iterMCMC=0;iterMCMC<NMCMC;iterMCMC++){
      ...
      if (prior[0]==0){
        ...
        for(l=0;l<n;l++){
          ...
          for (i=0;i<l;i++) {
            ...
            for (h=0;h<H;h++){
              prob_M[lambda[l]+n*h]=p[cumdime[h]+V[l+n*h]];
            }
          }
          for (h=0;h<H;h++){
            pcluster_M[lambda[l]+n*h]=prob_M[lambda[l]+n*h]; 
          }
        }
      }
      for(j=0;j<n;j++){
        if (unif_rand()<up_lambda[0]){
              ...
              for (h=0;h<H;h++){
                ...
                for ( i=0;i<n;i++){
                  if (i!=j  && lambda[i]==lambda[j]){
                    prob_M[lambda[j]+n*h]=prob_M[lambda[j]+n*h]*( (V[i+n*h]==V[j+n*h])*(1-a[h*n+lambda[j]])+a[h*n+lambda[j]]*p[cumdime[h]+V[i+n*h] ]); 
                  }
                }
                pcluster_M[lambda[j]+n*h]=(pcluster_M[lambda[j]+n*h]-(1-a[h*n+lambda[j]])*p[cumdime[h]+V[j+n*h] ]* prob_M[lambda[j]+n*h])/(a[h*n+lambda[j]]*p[cumdime[h]+V[j+n*h] ]);
              }
            }
            if (clsize[lambda[j]]==1) {
              for (h=0;h<H;h++){
          ...
          for(q=0; q<n; q++) {
            ...
            if (prior[0]==0) {
              for (h=0;h<H;h++){
                ...
                for ( i=0;i<n;i++){
                  if (i!=j  && lambda[i]==q){
                    prob_M[q+n*h]=prob_M[q+n*h]*((V[i+n*h]==V[j+n*h])*(1-a[h*n+q])+a[h*n+q]*p[cumdime[h]+V[i+n*h] ]);
                  }
                }
          ...
          for (i = 1 ; i < n; i++) psamp[i] += psamp[i - 1];
          ...
          lambda[j]= perm[l];
          for (h=0;h<H;h++){
            pcluster_M[lambda[j]+n*h]=prob_M[lambda[j]+n*h];
          }
          clsize[lambda[j]]=clsize[lambda[j]]+1;
          if (clsize[lambda[j]]==1){
            ksize=ksize+1;
    ...
    if (par_up[1]==1) {
          ...
          for (h=0;h<H;h++){
            ...
            for ( i=0;i<j;i++){
              if (lambda[i]==lambda[j]) {
                prob_M_new[lambda[j]+n*h]=prob_M_new[lambda[j]+n*h]*( (V[i+n*h]==V[j+n*h])*(1-a_prop[h*n+lambda[j]])+a_prop[h*n+lambda[j]]*p[cumdime[h]+V[i+n*h] ]);
                prob_M_old[lambda[j]+n*h]=prob_M_old[lambda[j]+n*h]*( (V[i+n*h]==V[j+n*h])*(1-a[h*n+lambda[j]])+a[h*n+lambda[j]]*p[cumdime[h]+V[i+n*h] ]);
              }  
            }
            prob_M_new[lambda[j]+n*h]=a_prop[h*n+lambda[j]]*p[cumdime[h]+V[j+n*h]]*pcluster_M_new[lambda[j]+n*h]+(1-a_prop[h*n+lambda[j]])*p[V[j+n*h]+cumdime[h]]*prob_M_new[lambda[j]+n*h];
            prob_M_old[lambda[j]+n*h]     =a[h*n+lambda[j]]*p[cumdime[h]+V[j+n*h]]*pcluster_M_old[lambda[j]+n*h]+(1-     a[h*n+lambda[j]])*p[V[j+n*h]+cumdime[h]]*prob_M_old[lambda[j]+n*h];
      ...
      for (l=0;l<n;l++){
        for(h=0;h<H;h++){
          a_prop[h*n+l]=exp(logitadif[h*n+l]+logitmean_prop[h])/(1+exp(logitadif[h*n+l]+logitmean_prop[h]));
        }
      }
      for(h=0;h<H;h++) {
      ...
      for (h=0;h<H;h++){
        if (unif_rand()<exp(logpcond_a_new[h]-logpcond_a_old[h]+ dnorm(logitmean_prop[h],log(sigmal[2]/(1-sigmal[2])),sqrt(sigmal[1]),1) - dnorm(logitmean[h],log(sigmal[2]/(1-sigmal[2])),sqrt(sigmal[1]),1))){
    ...
    if (par_up[2]==1) {
      ...
      for (h=0;h<H;h++){
        ...
        for (k=0;k<dime[h];k++) {p_prop[cumdime[h]+k]=p_prop[cumdime[h]+k]/s_prop;
      ...
      for(j=1;j<n;j++){
        ...
        if (old==1){
          for (h=0;h<H;h++){
            ...
            for ( i=0;i<j;i++){
              if (lambda[i]==lambda[j]) {
                prob_M_new[lambda[j]+n*h]=prob_M_new[lambda[j]+n*h]*( (V[i+n*h]==V[j+n*h])*(1-a[h*n+lambda[j]])+a[h*n+lambda[j]]*p_prop[cumdime[h]+V[i+n*h] ]);
                prob_M_old[lambda[j]+n*h]=prob_M_old[lambda[j]+n*h]*( (V[i+n*h]==V[j+n*h])*(1-a[h*n+lambda[j]])+a[h*n+lambda[j]]*p[cumdime[h]+V[i+n*h] ]);
              }  
            }
            prob_M_new[lambda[j]+n*h] = a[h*n+lambda[j]]*p_prop[cumdime[h]+V[j+n*h]]*pcluster_M_new[lambda[j]+n*h]+(1-a[h*n+lambda[j]])*p_prop[V[j+n*h]+cumdime[h]]*prob_M_new[lambda[j]+n*h];
            prob_M_old[lambda[j]+n*h] =a[h*n+lambda[j]]*p[cumdime[h]+V[j+n*h]]*pcluster_M_old[lambda[j]+n*h]+(1-     a[h*n+lambda[j]])*p[V[j+n*h]+cumdime[h]]*prob_M_old[lambda[j]+n*h];
            logpcond_aSC_new[lambda[j]+n*h]=log(prob_M_new[lambda[j]+n*h]/pcluster_M_new[lambda[j]+n*h]);
            logpcond_aSC_old[lambda[j]+n*h]=log(prob_M_old[lambda[j]+n*h]/pcluster_M_old[lambda[j]+n*h]); 
          }
      ...
      for (h=0;h<H;h++){
        lqratio=0.0;
        for (k=0;k<dime[h];k++) {
          lqratio=lqratio+(cdir*p[cumdime[h]+k])*log(p_prop[cumdime[h]+k])-(cdir*p_prop[cumdime[h]+k])*log(p[cumdime[h]+k])+lgamma(cdir*p_prop[cumdime[h]+k]+1.0)-lgamma(cdir*p[cumdime[h]+k]+1.0);
        }
        if (unif_rand()<exp(logpcond_a_new[h]-logpcond_a_old[h]+lqratio)){
          for (k=0;k<dime[h];k++){
            p[cumdime[h]+k]=p_prop[cumdime[h]+k];
          }
        }
      }
    }    
    if (par_up[3]==1) {
      ...
      for (i = 0 ; i < Nmax; i++){
        psampN[i] = psampN[i]/psampN[Nmax-1];
      }
      rU = unif_rand();
      for (l = 0; l < (Nmax-1); l++) {
          if (rU <= psampN[l]){
    ...
    for (h=0;h<H;h++){
      ...
      for (k=0;k<dime[h];k++){
        simp[totdime*iterMCMC+cumdime[h]+k]=p[cumdime[h]+k];
      }
    }
    for (i=0;i<n;i++){
      simlambda[n*iterMCMC+i]=lambda[i];
    }
    Rprintf("\n ");
  }
  PutRNGstate();
