...
coupleN <- function(N1, N2, k1,k2, g, n){
  ...
  lweights1 <- sapply(1:maxN, function(xx) lfactorial(xx) - lfactorial(xx - k1) + (n+g) * log(xx) )
  lweights2 <- sapply(1:maxN, function(xx) lfactorial(xx) - lfactorial(xx - k2) + (n+g) * log(xx) )
  ...
  cp_n_res <- couple_multinomial_alt(p = w1, q = w2, n = maxN)
  return(cp_n_res)
}
