...
update_theta <- function(theta, partition, alpha){
  ...
  for (l in 1:L){
  	## current state
    x <- theta[[l]] 
    cl_log_lik <- compute_loglikelihood_clusters_field(l - 1, partition, x, V, dimV, alpha)
    ## dirichlet proposal
    x_propose <- gtools::rdirichlet(1, alpha = (1 + concentration * x))[1,]
    cl_log_lik_new <- compute_loglikelihood_clusters_field(l - 1, partition, x_propose, V, dimV, alpha)
    ## transition ratio 
