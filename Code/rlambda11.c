 ...
 double simp[])
{
  ...
  for (iterMCMC=0;iterMCMC<NMCMC;iterMCMC++){
        ...
        for(l=0;l<n;l++){
          ...
          if (old==1) {
            for (h=0;h<H;h++){
              ...
              for ( i=0;i<l;i++){
                if (lambda[i]==lambda[l]) prob_M[lambda[l]+n*h]=prob_M[lambda[l]+n*h]*((V[i+n*h]==V[l+n*h])*(1-a[h*n+lambda[l]])+a[h*n+lambda[l]]*p[cumdime[h]+V[i+n*h] ]);
              }
          ...
          if (prior[0]==0){
            if (clsize[lambda[j]]>1) {
              for (h=0;h<H;h++){
                ...
                for ( i=0;i<n;i++){
                  if (i!=j  && lambda[i]==lambda[j]) prob_M[lambda[j]+n*h]=prob_M[lambda[j]+n*h]*( (V[i+n*h]==V[j+n*h])*(1-a[h*n+lambda[j]])+a[h*n+lambda[j]]*p[cumdime[h]+V[i+n*h] ]); 
                }
            ...
            if (clsize[lambda[j]]==1) {
              for (h=0;h<H;h++) pcluster_M[lambda[j]+n*h]=1.0;
            }
          }
    ...
    if (par_up[1]==1) {
      ...
      for (l=0;l<n;l++){
        for (h=0;h<H;h++){
          if (unif_rand() < exp(logpcond_a_new[h*n+l]-logpcond_a_old[h*n+l] + dnorm(logitadif_prop[h*n+l],0,sqrt(sigmal[0]),1) - dnorm(logitadif[h*n+l], 0,sqrt(sigmal[0]),1) )){
