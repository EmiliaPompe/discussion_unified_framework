#'@title update_beta
#'@description update 'beta' component of the chain, that is both beta_0 and beta_diff
#'@return a state with updated 'beta_0', 'beta_diff', 'alpha' and updated loglikelihood
#'@export 
  ...
  for (icluster in 1:n){
    ...
    if (state$partition$clsize[icluster] <= 1){
      ...
      for (field in 1:p){
        ...
        proposed_beta_diff <- rnorm(1, current_beta_diff, sd = algotuning$proposal_sd/max(1,state$partition$clsize[icluster]))
        ## calculating the corresponding value of alpha
        exp_beta__ <- exp(state$beta_0[field] + proposed_beta_diff)
        proposed_alpha <- exp_beta__/(exp_beta__+1)
...
## alternative, integrating out what can be integrated out
#'@export
update_beta_collapsed <- function(state, hyper, V, algotuning){
  n <- dim(V)[1]
  p <- dim(V)[2]
  ## using the formula for the conjugate posterior in the Normal-Normal model
  nmorethan1 <- sum(state$partition$clsize>1)
  common_var <- 1/(1/hyper$s0_sq + nmorethan1/hyper$s_sq)
  for (field in 1:p){
    # calculating beta_prime as previous_beta_0 + new differences
    beta_prime <- state$beta_0[field] + state$beta_diff[state$partition$clsize>1,field]
    # posterior mean in the Normal-Normal model
    mu <- common_var * (hyper$m0/hyper$s0_sq + sum(beta_prime)/hyper$s_sq)
    state$beta_0[field] <- rnorm(1, mean = mu, sd = sqrt(common_var))
  }
  ...
  for (icluster in 1:n){
    ...
    if (state$partition$clsize[icluster] <= 1){
      ## this cluster is empty or contains only one element so likelihood
      ## does not depend on beta parameter; thus beta follows the prior
      proposed_beta_diff <- rnorm(p, mean = 0, sd = hyper$s)
      exp_beta_ <- exp(state$beta_0+proposed_beta_diff)
      proposed_alpha <- exp_beta_/(exp_beta_+1)
      state$beta_diff[icluster,] <- proposed_beta_diff
      state$alpha[icluster,] <- proposed_alpha
    } else {
      ## cluster has at least 2 elements, we need to take the likelihood into account
      # loop over fields
      for (field in 1:p){
        ...
        proposed_beta_diff <- rnorm(1, current_beta_diff, sd = algotuning$proposal_sd/max(1,state$partition$clsize[icluster]))
        ## calculating the corresponding value of alpha
        exp_beta_ <- exp(state$beta_0[field] + proposed_beta_diff)
        proposed_alpha <- exp_beta_/(exp_beta_+1)
        ## random number for acceptance/rejection
        logu <- log(runif(1))
        ## log likelihood taken from partition_ll + the prior
  ...
  for (icluster in 1:n){
    ...
    if ((state1$partition$clsize[icluster] > 1) || (state2$partition$clsize[icluster] > 1)){
      ...
      for (field in 1:p){
        ...
        ## we need to compute the cluster log-likelihood
        proposals_ <- maxcoupling_normal(current_beta_diff1, current_beta_diff2, algotuning$proposal_sd/max(1,state1$partition$clsize[icluster]),
                                         algotuning$proposal_sd/max(1,state2$partition$clsize[icluster]))
        proposed_beta_diff1 <- proposals_$xy[1]
        proposed_beta_diff2 <- proposals_$xy[2]
        ## calculating the corresponding value of alpha
        exp_beta_1 <- exp(state1$beta_0[field] + proposed_beta_diff1)
        exp_beta_2 <- exp(state2$beta_0[field] + proposed_beta_diff2)
        proposed_alpha1 <- exp_beta_1/(exp_beta_1+1)
        proposed_alpha2 <- exp_beta_2/(exp_beta_2+1)
        ## random number for acceptance/rejection
        logu <- log(runif(1))
        ## log likelihood taken from partition_ll + the prior
