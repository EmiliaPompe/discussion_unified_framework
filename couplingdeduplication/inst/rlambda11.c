                                                                                                                                                                                                         ...
                                                                                                                                                                                                         double simp[])
{
  ...
  for (l=0;l<n;l++){
    for(h=0;h<H;h++){logita[h*n+l]=log(a[h*n+l]/(1-a[h*n+l]));
  ...
  for (iterMCMC=0;iterMCMC<NMCMC;iterMCMC++){
    ...
    if (par_up[0]==1){
      ...
      if (prior[0]==0){
        for (l=0;l<(H*n);l++) {pcluster_M[l]=1.0;
          prob_M[l]=1.0;}
        ...
        for(l=0;l<n;l++){
          ...
          else { for (h=0;h<H;h++) prob_M[lambda[l]+n*h]=p[cumdime[h]+V[l+n*h]];}
      ...
      for(j=0;j<n;j++){
        if (unif_rand()<up_lambda[0]){
          ...
          for(q=0; q<n; q++) {
            ...
            if (prior[0]==1){ if ( clsize[q]==0 ||(clsize[q]==1 && q==lambda[j])) 	psamp[q]=psamp[q]*(N-ksize+0.0)/(n-ksize+0.0);}
    ...
    if (par_up[1]==1) {
      ...
      for (h=0;h<H;h++){
        if (unif_rand()<exp(logpcond_a_new[h]-logpcond_a_old[h]+
            ...
            dnorm(logitmean[h],log(sigmal[2]/(1-sigmal[2])),sqrt(sigmal[1]),1))) logitmean[h]=logitmean_prop[h];}
      for (l=0;l<n;l++){
    ...
    if (par_up[2]==1) {
      ...
      for (h=0;h<H;h++){
        lqratio=0.0;
        for (k=0;k<dime[h];k++) {lqratio=lqratio+(cdir*p[cumdime[h]+k])*log(p_prop[cumdime[h]+k])-(cdir*p_prop[cumdime[h]+k])*log(p[cumdime[h]+k])+
          lgamma(cdir*p_prop[cumdime[h]+k]+1.0)-lgamma(cdir*p[cumdime[h]+k]+1.0);}
        
        if (unif_rand()<exp(logpcond_a_new[h]-logpcond_a_old[h]+lqratio)){ for (k=0;k<dime[h];k++) p[cumdime[h]+k]=p_prop[cumdime[h]+k];}}
